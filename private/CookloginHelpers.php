<?php
	...
	function createAccount($user, $pw, $UserName, $email){
		...
		if($myHandle){
			$stmt = $myHandle->prepare("SELECT count(*) as total FROM Cook WHERE userid=:u_id");
			...
			if($count == 0){
				...
				$sql = "INSERT into Cook (userid, password, UserName, email) VALUES(:u_id, :p_word, :UserName, :mail);";
	...
	function attempt_login($userID, $passwordAttempt){
		...
		if($myHandle){		
			$myStmt = $myHandle->prepare("SELECT userid, password FROM Cook WHERE userid=:u_id");
